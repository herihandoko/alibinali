...
//            'business_name'              => 'required|max:255',
        ...
        $request->validate($rules,[
            'business_name.required'=>__("Nama Perusahaan belum diinput")
        ]);
        ...
        if ($row->save()) {
            return back()->with('success', ($id and $id>0) ? __('User diperbarui'):__("User dibuat"));
        }
    }
    ...
    public function userUpgradeRequest(Request $request)
    {
        ...
        $data = [
            'rows' => $listUser->whereHas('user')->with(['user', 'role', 'approvedBy', 'referral'])->orderBy('id','desc')->paginate(20),
            'roles' => Role::all(),
    ...
    public function userUpgradeRequestApproved(Request $request)
    {
        ...
        $action = $request->input('action');
        if (empty($ids))
            return redirect()->back()->with('error', __('Silakan pilih salah satu user!'));
        if (empty($action))
            return redirect()->back()->with('error', __('Select an Action!'));
        ...
        switch ($action){
            case "delete":
                ...
                return redirect()->back()->with('success', __('Berhasil dihapus!'));
                break;
            default:
                foreach ($ids as $id) {
                    $vendorRequest = VendorRequest::find( $id);
                    if(!empty($vendorRequest)) {
                        $userReferralCheck = VendorReferral::find($vendorRequest->referral_id);
                        if(empty($userReferralCheck)) {
                            $lastCodeAuto = VendorReferral::latest('code_auto')->first();
                            ...
                            if(empty($lastCodeAuto)) {
                                $codeAuto = 'ALI'.$year.$month.'0001';
                            } else {
                                $numericId = $lastCodeAuto->code_auto;
                                $numericId++;
                                
                                if(mb_strlen($numericId) == 1) {
                                    $zeroString = '000';
                                } elseif(mb_strlen($numericId) == 2) {
                                   $zeroString = '00';
                                } elseif(mb_strlen($numericId) == 3) {
                                   $zeroString = '0';
                                } else {
                                   $zeroString = '';
                                }
                                $codeAuto = 'ALI'.$year.$month.$zeroString.$numericId;
                            }
                            
                            $saveUserReferral = new VendorReferral();
                            $saveUserReferral->code_auto = $numericId;
                            $saveUserReferral->referral_code = $codeAuto;
                            $saveUserReferral->user_id = $vendorRequest->user_id;
                            $saveUserReferral->save();
                            
                            if($saveUserReferral->id != 0) {
                                $vendorRequest->update(['status' => $action, 'referral_id' => $saveUserReferral->id, 'approved_time' => now(), 'approved_by' => Auth::id()]);
                                ...
                                return redirect()->back()->with('success', __('Berhasil diperbarui!'));
                            } else {
                                return redirect()->back()->with('error', __('Gagal diperbarui!'));
                            }
                        }
                    }
                }
                return redirect()->back()->with('error', __('Sudah disetujui sebagai Agent!'));
                break;
        }
    }
    public function userUpgradeRequestApprovedId(Request $request, $id)
    {
        ...
        if(!empty($vendorRequest)){
            ...
            if(empty($userReferralCheck)) {
                ...
                return redirect()->back()->with('error', __('User sudah memiliki kode referral!'));
            }
        }
    }
