...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    {
        if (!empty($this->idcard_id)) {
            return get_file_url($this->idcard_id, 'thumb');
        }
        if (!empty($meta_avatar = $this->getMeta("social_meta_avatar", false))) {
            return $meta_avatar;
        }
        return asset('images/idcard.png');
    }
    public function getIdcardUrlAttribute()
    {
        return $this->getIdcardUrl();
    }

    public function getFamilycardUrl()
    {
        if (!empty($this->familycard_id)) {
            return get_file_url($this->familycard_id, 'thumb');
        }
        if (!empty($meta_avatar = $this->getMeta("social_meta_avatar", false))) {
            return $meta_avatar;
        }
        return asset('images/familycard.png');
    }
    public function getFamilycardUrlAttribute()
    {
        return $this->getFamilycardUrl();
    }

    public function getPassportUrl()
    {
        if (!empty($this->passport_id)) {
            return get_file_url($this->passport_id, 'thumb');
        }
        if (!empty($meta_avatar = $this->getMeta("social_meta_avatar", false))) {
            return $meta_avatar;
        }
        return asset('images/passport.png');
    }
    ...
    public function sendPasswordResetNotification($token)
    {
        ...
        $updateUser = User::where('id', $this->id)->update([
        ...
        if ($updateUser) {
            ...
            try {
                event(new SendWhatsAppUserResetPassword($user));
            } catch (Exception $exception) {
                Log::warning("SendWhatsAppUserResetPassword: " . $exception->getMessage());
            }
        }
    ...
    {
        return DB::table('user_upgrade_request')
            ->select('vendor_referral.referral_code')
            ->join('vendor_referral', 'user_upgrade_request.referral_id', 'vendor_referral.id')
            ->where('user_upgrade_request.user_id', $this->id)->first();
    }
