...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            ...
            <table class="table table-bordered table-striped table-sm" data-toggle="table" data-search="true"
                data-show-columns="true" width="100%" cellspacing="0" style="font-size: small;">
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        ...
                                        <a class="dropdown-item"
                                            href="{{ route('vendor.team.edit', ['vendorTeam' => $vendorTeam->member->id]) }}">{{ __('Ubah') }}</a>
                                        @if ($vendorTeam->status == Modules\Vendor\Models\VendorTeam::STATUS_PENDING)
                                            <a class="dropdown-item"
                                                href="{{ route('vendor.team.re-send-request', ['vendorTeam' => $vendorTeam]) }}">{{ __('Send email') }}</a>
                                        @endif
                                        <a class="dropdown-item"
                                            href="{{ URL::signedRoute('vendor.team.delete', ['vendorTeam' => $vendorTeam->member->id]) }}">{{ __('Hapus') }}</a>
                                    </div>
                                </div>
                            </td>
                        </tr>
